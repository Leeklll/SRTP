#
#lookbusy下载地址：http://freshmeat.sourceforge.net/projects/lookbusy
...
def freq_set(f):
	num=cores_on_num()
	for i in range(num):
		subprocess.call('cpufreq-set -c %s -u %s'%(str(i), str(f)), shell=True)
		subprocess.call('cpufreq-set -c %s -d %s'%(str(i), str(f)), shell=True)

#FUNC 3: 给当前开启的核心进行加压，参数为加压百分比
def stress(s,t,tslp):
  lookbusy_p=subprocess.Popen('lookbusy -c %s --quiet'%(str(s)), shell=True)
  time.sleep(t)
  subprocess.call('pkill -9 lookbusy', shell=True)#杀死lookubusy加压进程
  time.sleep(tslp)

#def cpustress(n,f,s,t,tslp):
...
def memorystress(z,t,tslp):       #内存加压，
...
def networkstress(network,TCPwindow,j,t,tslp):
  ...
  os.system(str_sys%(network,TCPwindow))            #TCP模式加压
  time.sleep(tslp)
...
#a--cores, b--freq, c--cpustress, d-memstress, e--diskstress, f--netstress


  choose=input("Do you want to choose automatic? please input No or Yes.if you choose Yes,you need input the initial data,but you input the you need data:")
  t=input("please input the time you want to stress:")            #设置加压时长
  tslp=int(t)+2                                         #sleep时长比加压多出来5秒

  network=input("please,input the ip you will transfer:")        #传输数据的目标ip
  TCPwindow=input("please, input the TCP has the initial bandwidth:")#测电脑TCP协议下最大带宽
  lshw()
  if choose=="No":
     cpucores=input("please input the cpu cores number:")#设置cpu核心数
     e=input("please input the disk stress:")#设置disk加压序数
     s=input("please input the cpu stress:")             #设置对cpu核心加压百分     
     z=input("please,input the memused you want to stress:")    ##输入要设置的加压百分比

     cpu=int(cpucores)
     if int(cpu)>cores_num or int(cpu)==0:
           cores_on(cpu)
     frequency=input("please input the cpu frequency:")#设置频率
     pool=ThreadPoolExecutor(max_workers=6)
     freq_set(frequency)
     pool.submit(stress,s,t,tslp)
    
     pool.submit(memorystress,z,t,tslp)
     pool.submit(diskstress,e,t)
     
     pool.submit(networkstress,network,TCPwindow,j,t,tslp)
  ...
  if choose== "Yes":
    for a in range(0,8): 
      A=a+1   #开启a+1个核心1-8
      for b in range(0,10):
          B=1200+100*b   #设置所开启的核心数频率1200-2100
          for c in range(0,10):
              C=10*(c+1)    #给当前cpu加压,10%-100%
              for d in range(0,7):
                  D=10*(d+4)     #给内存加压，40%-100%(初始一般30%多)
                  for e in range(0,11):   #---------------------------e---------#             
                                 for f in range(0,10):
                                     ...
                                     cores_on(A)
                                     freq_set(B)
                                     pool.submit(stress,C,t,tslp)
                                     pool.submit(memorystress,D,t,tslp)
                                     pool.submit(diskstress,e,t)
                                     pool.submit(networkstress,network,TCPwindow,F,t,tslp)
                                     sar()
                                     pool.shutdown(wait=True)
#END                                     
